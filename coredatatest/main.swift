...
func getAllPossibleRoutes3(start: String, end: String) -> [Route] {
    ...
    // ------- replace this part ------
    func findAllRoutesWithLineChanges(start: String, end: String, adjacencyList: [String: [String]], stationLineColors: [String: String]) -> [[String]] {
        var firstMinLineChanges: Int?
        var secondMinLineChanges: Int?
        var firstMinResult: [[String]] = []
        var secondMinResult: [[String]] = []
        ...
        while !queue.isEmpty {
            ...
            if currentNode == end {
                if firstMinLineChanges == nil || lineChanges <= firstMinLineChanges! {
                    if lineChanges < firstMinLineChanges ?? Int.max {
                        // Clear previous results if the current line change is less
                        secondMinLineChanges = firstMinLineChanges
                        secondMinResult = firstMinResult
                        firstMinLineChanges = lineChanges
                        firstMinResult = []
                    }
                    firstMinLineChanges = lineChanges
                    firstMinResult.append(path)
                } else if secondMinLineChanges == nil || lineChanges <= secondMinLineChanges! {
                    if lineChanges < secondMinLineChanges ?? Int.max {
                        // Clear previous results if the current line change is less
                        secondMinLineChanges = lineChanges
                        secondMinResult = []
                    }
                    secondMinLineChanges = lineChanges
                    secondMinResult.append(path)
                }
                continue
            }
        ...
        return (firstMinResult + secondMinResult).prefix(3).map { $0 }
    }
    // ------- replace this part ------
    ...
    // ------- replace this part ------
    let result = findAllRoutesWithLineChanges(start: start, end: end, adjacencyList: adjacencyList, stationLineColors: stationLineColors)
    // ------- replace this part ------
...
print("----------------")
print("output first and second minimum line change routes (limit to 3) ")
print("----------------")


let testresult = getAllPossibleRoutes3(start: "A1", end: "A8")
for route in testresult {
    for station in route.stations {
        print(station.station_id, station.station_name)
    }
...
print("----------------")
print("output first minimum line change routes only (limit to 3) ")
print("----------------")
