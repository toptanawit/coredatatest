...
func getAllPossibleRoutes(start: String, end: String) -> [Route] {
    ...
    for routes in result {
        ...
        for route in routes {
            let fetchRequest = NSFetchRequest<NSFetchRequestResult>(entityName: "Stations") // Replace with your actual entity name
            fetchRequest.predicate = NSPredicate(format: "station_id == %@", route)

            do {
                let fetchedStations = try context.fetch(fetchRequest) as! [NSManagedObject]

                if let firstStation = fetchedStations.first {
                    let tempStation = Station(station_id: firstStation.value(forKey: "station_id") as? String ?? "",
                                              station_name: firstStation.value(forKey: "station_name") as? String ?? "",
                                              station_line: firstStation.value(forKey: "station_line") as? String ?? "",
                                              station_linecolor: firstStation.value(forKey: "station_linecolor") as? String ?? "",
                                              latitude: firstStation.value(forKey: "latitude") as? Double ?? 0.0,
                                              longitude: firstStation.value(forKey: "longitude") as? Double ?? 0.0,
                                              is_extended: firstStation.value(forKey: "is_extended") as? Bool ?? false)
                    

                    stationsData.append(tempStation)
                }
            } catch {
                print("Failed to fetch data for station ID \(route): \(error)")
            }
        }
